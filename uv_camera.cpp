...
const float cam_pi = 4.0f*atan(1.0f);
const float cam_pi_half = 0.5f* cam_pi;
const float cam_pi_2 = 2.0f* cam_pi;
const float cam_epsilon = 1e-6f;
...
void uv_camera::Set(const float u_rad, const float v_rad, const float w_metres, const float fov_deg, const int width_px, const int height_px, float src_near, float src_far)
{
	...
	static const float lock = cam_epsilon * 1000.0f;

	if(u < -cam_pi_half + lock)
		u = -cam_pi_half + lock;
	else if(u > cam_pi_half - lock)
		u = cam_pi_half - lock;
...
void uv_camera::Transform(void)
{
	...
	gluLookAt(
		...
		eye.x.convert_to<double>() + look_at.x.convert_to<double>(), eye.y.convert_to<double>() + look_at.y.convert_to<double>(), eye.z.convert_to<double>() + look_at.z.convert_to<double>(), // Look at position (not direction).
...
void uv_camera::Set_Large_Screenshot(size_t num_cams, size_t cam_index_x, size_t cam_index_y)
{
    ...
    const float deg_to_rad = (1.0/360.0)*2* cam_pi;
